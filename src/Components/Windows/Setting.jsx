		...
		<div className="flex flex-wrap w-full">
			{displayBgs.map((bg, index) => (
				<button key={index} onClick={() => handleClick(bg.color ? bg.color : bg.img)} className={`w-1/4 h-full mx-4 justify-center items-center`}>
					{bg.color ? <div className={`${bg.color} min-w-full h-1/2`}></div> : <img className="h-1/2 w-full" src={bg.img} />}
			  	</button>
			))}
		</div>
	);
...
const   ColorsSettings = ({ bgType }) => {
	...
	return <div className="flex flex-row h-full w-full"><div className="flex flex-col justify-center border-r-white items-center h-auto mx-4">
